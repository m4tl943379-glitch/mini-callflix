...
- **Design system cinématique** (`client/src/styles.css`) : tokens dans `:root` (`--accent: #e50914`, `--bg-0…3`, `--r-s…xl`, `--ease`, `--m-*` durations, `--sh-*`/`--glow`). Ambiance salle : fond quasi-noir, lumières radiales rouges/bordeaux (`movie-scene`, `.room-page::before`), vignette interne, grain SVG (`cf-grain`/`cf-cine-grain`), glassmorphism modéré (backdrop-blur). Hiérarchie visuelle : MOVIE (scene centrale) > CAM (sidebar 370px) > Feeling. Le **Room Container `.sidebar`** (desktop) est un **panneau épuré sans bordure ni carte** (pas de border/background/box-shadow/radius — le fond du bottom sheet mobile lui est rendu explicitement dans la media query mobile). Les caméras `.cam-chip` gardent leurs cartes cinématiques. Le **bouton chat fermé** `.chat-icon-btn` est un **bouton flottant circulaire discret 42px en bas-droite** du container (`right/bottom: 14px`, désaxé des caméras et des bords, `display:none`→`opacity 0` quand chat ouvert, que ce soit desktop, fullscreen — `right: 14px; bottom: 10px` — ou mobile `right/bottom: 16px`). Tous les boutons de contrôles/vidéo utilisent des **icônes SVG inline** (`MoviePlayer.jsx` : IconPlay/Pause/Vol/Mute/Reload/Fullscreen/Exclaim ; `App.jsx` : IconCam/Mic/Off/Chat/Fullscreen/Shrink) avec `aria-label` — aucun emoji dans les contrôles. Zones sensibles à préserver : `.cf-movie iframe` (YouTube pointer-events:none), `.chrome-hidden .cf-controls/.control-bar`, `.room-page:fullscreen` (topbar/sidebar invite-box masqués, `.chat-icon-btn` en bas-droite), `@media (prefers-reduced-motion)` réduit tout. Slider `.cf-range` avec thumb visible au hover.
...
- **Fullscreen 80/20 caméra/film** (`App.jsx` + `styles.css`) : en **fullscreen desktop uniquement**, le workspace passe en row strict `movie 80% | cam 20%` (`.watch-stage`/`.sidebar` `flex: 0 0 80%/20%`, `gap: 0`, padding 10px du `.room-page`). La **paire caméras est un groupe verrouillé** (stack vertical constant, jamais flottante/verticale dans le temps) : drag horizontal de `.cameras` (pointeur capturé, `camDragRef`, `camSide`/`camSideRef`) pour **basculer uniquement DROITE↔GAUCHE** — transition `transform .55s cubic-bezier(.22,1,.36,1)` + `transition:none` inline pendant le drag + `will-change` (Apple-like, aucun reload/reconnect, movie jamais re-monté). Math du drag : `camX ∈ [-0.8W, 0]`, `movieX = -camX/4`, snap à `camX < -0.4W` (~milieu) → classe `.workspace.cams-left` → `translateX(25%)` (stage) / `translateX(-400%)` (sidebar). Reset automatique à `right` à la sortie du fullscreen (cleanup inline transforms), `setSidebarOpen(true)` à l'entrée (caméras toujours visibles). Mobile fullscreen : sidebar masquée, movie 100% (override `flex:1; transform:none` dans la media query). Ne pas toucher à la synchro/WebRTC/chat.
